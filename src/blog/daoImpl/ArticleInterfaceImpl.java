...
public class ArticleInterfaceImpl implements ArticleInterface {
	...
	private static ArticleInterface instance;

	private ArticleInterfaceImpl() {
		conn = C3P0Connection.getInstance().getConnection();
	}

	public static final ArticleInterface getInstance() {
		if (instance == null) {
			try {
				instance = new ArticleInterfaceImpl();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	@Override
	public List searchArticle(String name) {
		List<Article> list = null;
		Article at = null;
